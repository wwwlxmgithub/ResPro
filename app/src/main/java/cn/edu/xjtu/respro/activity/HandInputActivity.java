...
public class HandInputActivity extends BaseActivity implements View.OnClickListener{
    ...
    private TextView time, subTotalPoints;
    ...
    private void checkPermissions() {
        if(checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions(HandInputActivity.this, new String[]{Manifest.permission.CAMERA}, REQ_CODE_REQUEST_PERMISSION);
        }else {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.submitBtn:
                ...
                submitPoints(recordId, array.toString(), (int)(subTotalPointsDouble * 1000), improper, (int)(deduct * 1000),
                        elect, (int)(totalP * 1000), note);
                break;
        }
    }

    private void submitPoints(Integer recordId, String detail, int sub, String improper, int deduct, String elect, int total, String note) {
        showUpload();
        NetworkRequestUtil.handInputPoints(recordId, detail, sub, improper, deduct, elect, total, note, "handInputPoints", new JSONObjectRequestListener() {
            @Override
            public void onResponse(JSONObject response) {
                try {
        ...
        switch (requestCode) {
            case REQ_SCAN_CODE:
                if (RESULT_OK == resultCode) {
                    ...
                    showGettingInfo();
                    NetworkRequestUtil.getRecordInfo(resultStr, "autoInputPoints", new JSONObjectRequestListener() {
                        @Override
                        public void onResponse(JSONObject response) {
                            try {
                                int ret = response.getInt(NetworkRequestUtil.RET);
                                if (ret == 0) {
                                    showInfo(response.getInt("id"), response.getLong("createTime"), response.getDouble("weight"));
                                    if (typeList == null) {
                                        getTrashTypes();
                                    }
                                    showTypes();
    ...
    private void showInfo(int id, long createTime, double weight) {
        recordId = id;
        infoLinearLayout.setVisibility(View.GONE);
        scanCodeBtn.setVisibility(View.GONE);
        scrollView.setVisibility(View.VISIBLE);
        time.setText(new SimpleDateFormat("yyyy-MM-dd HH:mm").format(createTime));
    ...
    class GridViewAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (convertView == null) {
                convertView = LayoutInflater.from(HandInputActivity.this).inflate(R.layout.grid_item, null, false);
            }
