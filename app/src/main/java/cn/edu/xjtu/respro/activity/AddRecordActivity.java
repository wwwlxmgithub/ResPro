...
public class AddRecordActivity extends BaseActivity implements View.OnClickListener{
    ...
    private TextView nickname, subTotalPoints;
    ...
    private void initView() {
        ...
        nickname = (TextView) findViewById(R.id.userTextView);
    ...
    private void checkPermissions() {
        if(checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions(AddRecordActivity.this, new String[]{Manifest.permission.CAMERA}, REQ_CODE_REQUEST_PERMISSION);
        }else {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.submitBtn:
                ...
                submitRecord(array.toString(), (int)(subTotalPointsDouble * 1000), improper, (int)(deduct * 1000),
                        elect, (int)(totalP * 1000), note);
                break;
        }
    }

    private void submitRecord(String detail, int sub, String improper, int deduct, String elect, int total, String note) {
        showUpload();
        NetworkRequestUtil.addRecord(userId, detail, sub, improper,deduct, elect, total, note, "addRecord", new JSONObjectRequestListener() {
            @Override
            public void onResponse(JSONObject response) {
                try {
        ...
        switch (requestCode) {
            case REQ_SCAN_CODE:
                if (RESULT_OK == resultCode) {
                    ...
                    try {
                        JSONObject ret = new JSONObject(new String(Base64.decode(resultStr, Base64.DEFAULT)));
                        userId = ret.getInt("id");
                        long createTime = ret.getLong("time");
                    ...
                    showGettingInfo();
                    NetworkRequestUtil.getUserInfo(userId, "getUserInfo", new JSONObjectRequestListener() {
                        @Override
                        public void onResponse(JSONObject response) {
                            try {
                                int ret = response.getInt(NetworkRequestUtil.RET);
                                if (ret == 0) {
                                    showInfo(response.getString(NetworkRequestUtil.RESULT));
                                    if (typeList == null) {
                                        getTrashTypes();
                                    }
                                    showTypes();
    ...
    private void showInfo(String nick) {
    ...
    class GridViewAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (convertView == null) {
                convertView = LayoutInflater.from(AddRecordActivity.this).inflate(R.layout.grid_item, null, false);
            }
